...
public class MeetingController {
    ...
    public ResponseEntity<?> addTranscriptLine(@RequestBody CreateTranscriptRequest request) {
        try {
            meetingService.addTranscriptLine(request);
            return ResponseEntity.ok("Transcript line added successfully");
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Failed to add transcript: " + e.getMessage());
        }
    }
